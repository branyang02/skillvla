"""
datasets/materialize.py

Factory functions for creating datasets.
"""

from pathlib import Path
from typing import Tuple, Type

from torch.utils.data import Dataset
from transformers import PreTrainedTokenizerBase

from skillvla.components.action_head.base_action_head import ActionHead
from skillvla.components.llm.prompting.base_prompter import PromptBuilder
from skillvla.components.vision.base_vision import ImageTransform
from skillvla.datasets.datasets import EpisodicRLDSDataset, RLDSBatchTransform, RLDSDataset
from skillvla.util.data_utils import PaddedCollatorForActionPrediction


def get_vla_dataset_and_collator(
    data_root_dir: Path,
    data_mix: str,
    image_transform: ImageTransform,
    tokenizer: PreTrainedTokenizerBase,
    action_head: ActionHead,
    prompt_builder_fn: Type[PromptBuilder],
    default_image_resolution: Tuple[int, int, int],
    padding_side: str = "right",
    predict_stop_token: bool = True,
    shuffle_buffer_size: int = 100_000,
    train: bool = True,
    episodic: bool = False,
    image_aug: bool = False,
) -> Tuple[Dataset, PaddedCollatorForActionPrediction]:
    batch_transform = RLDSBatchTransform(
        action_head, tokenizer, image_transform, prompt_builder_fn, predict_stop_token=predict_stop_token
    )
    collator = PaddedCollatorForActionPrediction(
        tokenizer.model_max_length, tokenizer.pad_token_id, padding_side=padding_side
    )

    # Build RLDS Iterable Dataset
    cls = RLDSDataset if not episodic else EpisodicRLDSDataset
    dataset = cls(
        data_root_dir,
        data_mix,
        batch_transform,
        resize_resolution=default_image_resolution[1:],
        shuffle_buffer_size=shuffle_buffer_size,
        train=train,
        image_aug=image_aug,
    )

    return dataset, collator
