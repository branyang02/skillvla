"""
Copied from https://github.com/lucidrains/vector-quantize-pytorch/blob/master/vector_quantize_pytorch/vector_quantize_pytorch.py
...
def kmeans(
    ...
    num_codebooks, dim, dtype, device = samples.shape[0], samples.shape[-1], samples.dtype, samples.device  # noqa: F841
...
class EuclideanCodebook(Module):
    ...
    def forward(
        ...
        if self.affine_param:
            ...
            embed = (embed - self.codebook_mean) * (batch_std / codebook_std) + self.batch_mean  # noqa: F821
...
class VectorQuantize(Module):
    def __init__(
        ...
        rotation_trick=True,  # Propagate grads through VQ layer w/ rotation trick: https://arxiv.org/abs/2410.06424 by @cfifty  # noqa: E501
        ...
        manual_ema_update=False,
        learnable_codebook=False,
        in_place_codebook_optimizer: Callable[  # noqa: RUF013
            ..., Optimizer
        ...
        # for variable lengthed sequences, whether to take care of masking out the padding to 0 (or return the original input)  # noqa: E501
    ...
    def forward(
        ...
        shape, device, heads, is_multiheaded, codebook_size, return_loss = (  # noqa: F841
            x.shape,
            x.device,
            self.heads,
            self.heads > 1,
        ...
        if self.training:
            ...
            if self.rotation_trick:
                ...
                rot_quantize = efficient_rotation_trick_transform(
                    safe_div(x, norm_x), safe_div(quantize, norm_quantize), x
                ).squeeze()
        ...
        # used for (1) naturalspeech2 training residual vq latents to be close to the correct codes and (2) cross-entropy based commitment loss  # noqa: E501
        ...
        if self.training:
            ...
            if self.has_codebook_orthogonal_loss:
                ...
                if self.orthogonal_reg_active_codes_only:
                    ...
                    ), "orthogonal regularization for only active codes not compatible with multi-headed with separate codebooks yet"  # noqa: E501
