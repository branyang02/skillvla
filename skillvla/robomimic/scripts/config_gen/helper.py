...
import robomimic
import skillvla.robomimic.utils.hyperparam_utils as HyperparamUtils

base_path = os.path.abspath(os.path.join(os.path.dirname(skillvla.robomimic.__file__), os.pardir))
...
def set_env_settings(generator, args):
    if args.env in ["droid"]:
        ...
        if "train.action_shapes" not in generator.parameters:
            generator.add_param(
...
def set_mod_settings(generator, args):
    if args.mod == 'ld':
        ...
        if "train.data_format" not in generator.parameters:
            generator.add_param(
